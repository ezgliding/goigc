...
BASE_PKG = github.com/ezgliding/goigc
...
$(BINDIR)/$(BINNAME): $(SRC)
	GO111MODULE=on go build $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(BINDIR)/$(BINNAME) ./cmd/goigc
...
dist:
	( \
		...
		$(DIST_DIRS) tar -zcf goigc-${VERSION}-{}.tar.gz {} \; && \
		$(DIST_DIRS) zip -r goigc-${VERSION}-{}.zip {} \; \
	)
